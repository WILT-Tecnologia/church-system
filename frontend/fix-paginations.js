...
files.forEach((file) => {
  if (fs.existsSync(file)) {
    ...
    content = content.replace(
      /^import\s*\{\s*MatPaginator\s*\}\s*from\s*'@angular\/material\/paginator';\r?\n/gm,
      '',
    );
    content = content.replace(
      /^import\s*\{\s*MatSort\s*\}\s*from\s*'@angular\/material\/sort';\r?\n/gm,
      '',
    );
    content = content.replace(
      /^[ \t]*@ViewChild\(MatPaginator\)\s*paginator!:\s*MatPaginator;\r?\n/gm,
      '',
    );
    content = content.replace(/^[ \t]*@ViewChild\(MatSort\)\s*sort!:\s*MatSort;\r?\n/gm, '');
    content = content.replace(
      /^[ \t]*this\.dataSourceMat\.paginator\s*=\s*this\.paginator;\r?\n/gm,
      '',
    );
